...
class BookingRepository {
    ...
    fun getServices(token: String, callback: (Result<List<ServiceResponse>>) -> Unit) {
        ...
        RetrofitClient.apiService.getServices("Bearer $token").enqueue(object : Callback<ServicesResponse> {
    ...
    fun getBarbers(token: String, callback: (Result<List<BarberResponse>>) -> Unit) {
        ...
        RetrofitClient.apiService.getBarbers("Bearer $token").enqueue(object : Callback<BarbersResponse> {
            override fun onResponse(
    ...
    fun getBarberAvailability(
        ...
        barberId: Int,
        date: String,
        callback: (Result<AvailabilityResponse>) -> Unit
    ) {
        Log.d(TAG, "Obteniendo disponibilidad para barbero $barberId en fecha $date")

        RetrofitClient.apiService.getBarberAvailability("Bearer $token", barberId, date).enqueue(object : Callback<AvailabilityResponse> {
            override fun onResponse(
